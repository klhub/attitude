...
%               be replaced by the text representation of the ordinal number
%               `value'.
%
...
%   the call:
%
%      [out] = cspice_repmot( in, marker, value, rtcase )
%
%   returns:
%
%      out      the string obtained by substituting the text representation
%               of the ordinal number `value' for the first occurrence of
...
%
%         [outstr] = cspice_repmot( instr, marker, 5, 'U' );
%
...
%
%         [outstr] = cspice_repmot( instr, marker, 5, 'L' );
%
...
%         [outstr] = cspice_repmot( outstr, marker, 5, 'C' );
%
...
%   by the calls
%
%      [string] = cspice_repmot( string, '#1', 51, 'L' );
...
%
%-&
function [out] = cspice_repmot( in, marker, value, rtcase )
   ...
   switch nargin
      ...
      otherwise
         ...
         error ( [ 'Usage: [`out`] = '                                      ...
                   'cspice_repmot( `in`, `marker`, value, `rtcase` )' ] )
   ...
   %
   try
      [out] = mice('repmot_c', in, marker, value, rtcase);
   catch spiceerr
      rethrow(spiceerr)
   end
